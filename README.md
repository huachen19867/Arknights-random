...
主界面按职业名额精确抽取：目标人数与方案名额一致，按钮固定两行“开始 / 抽取”，卡片左上角为黑色底白色职业图标，空缺槽位保留位置并显示对应职业。

![自选职业页](screenshots/profession-plan-page.png)

自选职业页支持重复添加同一职业、全选、逐槽删除，最多 12 个名额；底部提供与设置页一致的“启用自选职业 / 关闭自选职业”开关，启用后应用方案并返回抽取界面。

![设置页普通范围模式](screenshots/settings-range.png)

设置页职业范围区提供“启用自选职业 / 关闭自选职业”互斥开关；普通范围模式下职业多选与“全部选择 / 取消全选”保留。

![设置页自选职业模式](screenshots/settings-plan-locked.png)

启用自选职业后，人数与职业范围控件只读，摘要显示“自选职业模式 / N 个名额 / 职业×次数”，可跳转自选职业页修改方案。

![候选不足](screenshots/shortage-slots.png)

某职业候选不足时对应槽位留空并显示“候选不足”，状态条按职业汇总缺口，不跨职业补位。
...
PRTS 更新先生成本地种子，再预览与 Base 的差异。默认不会覆盖 Base 中已有记录，因此 Base 始终是人工维护源。一键编排会按顺序执行 PRTS 同步、真实样本检查、数据自检（以及有凭证时的 Base 三组 preview），并生成合并报告：

```powershell
npm.cmd run data:update:preview          # 完整预览（含飞书 Base 三组 preview，需 lark-cli）
npm.cmd run data:update:preview:no-base  # 无飞书凭证时只做 PRTS 同步 + 样本检查 + 数据自检
```
...
仓库还包含每周一次的只读检查工作流 [.github/workflows/check-data-updates.yml](.github/workflows/check-data-updates.yml)（也可手动触发）：它只执行 PRTS 同步、样本检查和数据自检，上传 `prts-diff.json` 产物；发现真实新增/变化或护栏失败时在步骤摘要中明确提示并让工作流失败，不会写 Base、不会提交生成物、不会自动合并。数据接受与 Base 写入始终在授权本机人工完成。
...
手写代码集中在 `src/`、`scripts/`、`config/`；职业图标资源放在 `public/assets/professions/`。以下文件是生成物：
...
干员卡左上角的八张职业图标来自 [PRTS Wiki](https://prts.wiki) 的 26 × 26 单色 PNG，已按职业重命名后随仓库发布在 `public/assets/professions/`，运行时不会热链 PRTS。项目属于非官方、非商业粉丝工具，游戏素材（含职业图标、立绘与干员信息）版权归鹰角网络/PRTS 相关权利人所有；发布时保留来源与权属声明。

PRTS 不是正式 API，页面结构变化可能破坏同步；脚本已使用真实样本检查、全量结构校验、稳定 ID 对账、版本迁移保护、有限重试和原子写入保护上一份可用数据。Base 批量更新还会只针对 `1254290` / `1254291` 或明确的方法级限流做指数退避。

内部交接说明、技术日志与验收证据只保存在本地 `docs/` 目录，不随仓库发布；可公开的复现说明以本文件为准。